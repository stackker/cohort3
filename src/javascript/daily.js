...
const functions = {
	...
	personStatistics: (peopleArr) => {
		let objStatistics = { countOfPeople: 0, sumOfAges: 0, avgOfAges: 0 };
		let statPeopleArr = peopleArr.map((objkt) => {
			objStatistics.countOfPeople += 1;
			objStatistics.sumOfAges += objkt.age;
		})
		objStatistics.avgOfAges = (objStatistics.sumOfAges / objStatistics.countOfPeople).toFixed(3);
		return objStatistics;
			...
			processPeopleFromProvince: (personArr, province, callBackFn) => {
				...
				// drawback is that it creates a seperate run for each province and testing has to be done inside an If Condition.
